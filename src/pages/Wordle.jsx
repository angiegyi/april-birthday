...
// ─── PUZZLE CONTENT ──────────────────────────────────────────────────────────
// Change ANSWER to any 5-letter word (all caps)
// Change AUTHOR to whoever submitted this puzzle
const ANSWER = 'APRIL' // ← CHANGE THIS
const AUTHOR = 'Your Friends' // ← CHANGE THIS
const HINT = 'Think about whose birthday it is today!' // ← CHANGE THIS
// ─────────────────────────────────────────────────────────────────────────────
...
export default function Wordle() {
  ...
  const [revealed, setRevealed] = useState([]) // array of booleans per completed row
      ...
      <div className="max-w-lg mx-auto">
            ...
            return (
              ...
              >
                {Array.from({ length: WORD_LENGTH }).map((_, colIdx) => {
                  ...
                  const state = rowIdx < guesses.length
                    ? getTileState(guess, colIdx, ANSWER)
                    : letter ? 'filled' : 'empty'
                  return (
                    <div
                      key={colIdx}
                      ...
                      style={
                        isRevealed && rowIdx < guesses.length
                          ? { transitionDelay: `${colIdx * 200}ms` }
                          : {}
                      }
                    >
                      {letter}
        ...
        {!gameOver && (
          <div className="text-center mb-4">
            ...
            {showHint && <p className="text-sm text-gray-500 italic mt-1">{HINT}</p>}
          </div>
        )}
